...
#pragma endregion
	...
	float percentage = fTimer / 3.0f;
	m_sProgrammer = "Zach Winnewisser - zmw7710@rit.edu"; //Replace with your name and email
	vector3 v3CurrentPos = glm::lerp(m_v3StopList[stopNum],m_v3StopList[(stopNum + 1) % m_v3StopList.size()],percentage); //Initialize this variable accordingly

	if (percentage >= 1.0f)
	{
		fTimer = 0;
		stopNum = (stopNum + 1) % m_v3StopList.size();
	}
